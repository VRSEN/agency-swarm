...
def test_tui_bridge_stream_config_ignores_request_model() -> None:
    """TUI bridge model placeholders must not replace the agency's configured model."""
    request = SimpleNamespace(app=SimpleNamespace(state=SimpleNamespace(agency_swarm_tui_bridge=True)))
    config = ClientConfig(model="gpt-5.4", api_key="sk-test", default_headers={"x-test": "1"})
