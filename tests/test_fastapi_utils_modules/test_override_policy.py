...
def test_build_file_upload_client_ignores_orcarouter_request_client(monkeypatch) -> None:
    from agency_swarm.utils.orcarouter import build_orcarouter_chat_model

    default = AsyncOpenAI(api_key="sk-openai", base_url="https://api.openai.com/v1")
    orcarouter = AsyncOpenAI(api_key="sk-orca-test", base_url="https://api.orcarouter.ai/v1")
    agent = SimpleNamespace(model=build_orcarouter_chat_model("orcarouter/openai/gpt-5", openai_client=orcarouter))
    agency = SimpleNamespace(agents={"A": agent}, entry_points=[SimpleNamespace(name="A")])
    ...
    policy = RequestOverridePolicy(ClientConfig(model="orcarouter/openai/gpt-5", api_key="sk-orca-test"))
    client = policy.build_file_upload_client(agency, recipient_agent="A")
