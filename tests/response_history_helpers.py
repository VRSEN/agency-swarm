...
class TrackingResponsesModel(Model):
    def __init__(self, model: str = "test-openclaw-threading", *, include_encrypted_reasoning: bool = False) -> None:
        self.model = model
    ...
    async def get_response(
        ...
        *,
        ...
        return _build_model_response(
            ...
            include_encrypted_reasoning=self.include_encrypted_reasoning,
        )
...
def _build_model_response(*, text: str, response_id: str, include_encrypted_reasoning: bool = False) -> ModelResponse:
    ...
    return ModelResponse(output=output, usage=usage, response_id=response_id)
...
def agency_factory_with_store(
    ...
    *,
    ...
    agent = Agent(
        ...
        model=model,
        model_settings=ModelSettings(store=False, temperature=0.0) if store_false else ModelSettings(temperature=0.0),
    )
    return Agency(
        agent,
...
def build_agency_factory(model: TrackingResponsesModel):
...
def build_store_false_agency_factory(model: TrackingResponsesModel):
    def create_agency(load_threads_callback=None, save_threads_callback=None):
        agent = Agent(
            name="TestAgent",
            instructions="Base instructions",
...
def assert_store_false_input_preserves_encrypted_reasoning(model_input: str | list[TResponseInputItem]) -> None:
    assert isinstance(model_input, list)
    reasoning_items = [item for item in model_input if isinstance(item, dict) and item.get("type") == "reasoning"]
