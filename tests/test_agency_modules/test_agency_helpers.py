...
def test_run_fastapi_creates_new_agency_instance(mocker):
    agent = Agent(name="HelperAgent", instructions="test", model="gpt-5.6-luna")
    agency = Agency(agent)
...
def test_run_fastapi_preserves_custom_tool_mappings(mocker):
    sender = Agent(name="A", instructions="test", model="gpt-5.6-luna")
    recipient = Agent(name="B", instructions="test", model="gpt-5.6-luna")
...
def test_run_fastapi_normalizes_relative_shared_folders_for_factory_calls(mocker, tmp_path: Path):
    ...
    creator_code = textwrap.dedent(
        ...
        a = Agent(name="A", instructions="test", model="gpt-5.6-luna")
