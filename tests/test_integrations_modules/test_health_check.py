...
class TestHealthCheckEndpoints:
    ...
    async def test_health_check_with_openai_failure(self, mock_openai_check, fastapi_app):
        ...
        mock_openai_check.return_value = AsyncMock(
            return_value=HealthCheckResult(
                name="openai_api", status="unhealthy", message="OpenAI API authentication failed", duration_ms=100.0
            )
        )
