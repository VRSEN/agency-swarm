...
async def test_concurrent_messages_to_same_agent():
    ...
    """
    gate = asyncio.Event()
    sender = Agent(name="Coordinator", instructions="Coordinate tasks", model="gpt-5.6-luna")
    recipient = Agent(name="Worker", instructions="Handle tasks", model="gpt-5.6-luna")
...
async def test_messages_to_different_agents():
    ...
    started = 0
    sender = Agent(name="Coordinator", instructions="Coordinate tasks", model="gpt-5.6-luna")
    recipient_one = Agent(name="Worker1", instructions="Handle tasks", model="gpt-5.6-luna")
    recipient_two = Agent(name="Worker2", instructions="Handle tasks", model="gpt-5.6-luna")
