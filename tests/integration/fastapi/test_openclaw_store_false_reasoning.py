...
@pytest.mark.skipif(
    os.getenv("CI") == "true",
    reason="Requires live OpenAI reasoning output; skipped on CI to avoid upstream flake.",
)
...
async def test_codex_chat_name_store_false_uses_encrypted_reasoning_include() -> None:
    ...
    class _TitleStream:
        def __aiter__(self: _TitleStream) -> _TitleStream:
            return self

        async def __anext__(self: _TitleStream) -> None:
