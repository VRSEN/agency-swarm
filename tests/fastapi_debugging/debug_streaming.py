...
def debug_streaming() -> None:
    ...
    with httpx2.Client(follow_redirects=True, timeout=None) as client:
        ...
        if response.status_code == 200:
            ...
            caller_agent_pct = caller_agent_count * 100 // max(event_count, 1)
            print(f"  Events with 'callerAgent': {caller_agent_count} ({caller_agent_pct}%)")
            ...
            # The response was opened with stream=True, so the body must be
            # read explicitly before .text is available.
            response.read()
            print(response.text)
