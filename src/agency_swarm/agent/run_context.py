...
The deprecated ``Agency.user_context`` sync-back path is isolated here so the
next breaking release can remove it without changing the run-scoped context
flow. ``MasterContext.user_context`` remains the single mutable store during a
run; ``context_override`` layers caller-owned state on top per run.
"""
...
def get_agency_user_context_store(agency_instance: object) -> dict[str, Any] | None:
    """Return the mutable context store backing the deprecated agency-level state.

    Lookup order keeps internal reads off the deprecated ``Agency.user_context``
    property: ``_shared_run_user_context`` is the live run dict carried by
    lightweight agency stand-ins (for example ``send_message`` recipients),
    ``_initial_user_context`` is the deprecated seed stored by ``Agency``, and a
    plain ``user_context`` attribute covers external agency stand-ins.
    """
...
def sync_context_back_to_agency(
    ...
    When ``context_override`` produced a merged copy of the run context, keys the
    run added beyond the override are written back to the deprecated agency store
    so existing integrations keep working until the next breaking release.
    """
