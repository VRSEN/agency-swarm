...
def _handle_raw_response(oe: Any, call_id_by_item: Dict[str, str]) -> BaseEvent | list[BaseEvent] | None:
    ...
    if etype == "response.output_item.added":
        ...
        # Tool call start
        if getattr(raw_item, "type", "") in _TOOL_TYPES:
            call_id, tool_name, _ = _tool_meta(raw_item)
            if not call_id:
    ...
    if etype == "response.output_item.done":
        ...
        if getattr(raw_item, "type", "") in _TOOL_TYPES:
            call_id, tool_name, arguments = _tool_meta(raw_item)
            if not call_id:
            ...
            return [
                ...
                _snapshot_event(raw_item.id, call_id, tool_name, arguments),
            ]

    # --- Tool-argument deltas ---------------------------------------------
    if etype in _TOOL_ARG_DELTA_TYPES:
        item_id = getattr(oe, "item_id", None)
...
def openai_to_agui_events(
    ...
    *,
    ...
    state = _RUN_STATE.setdefault(run_id, {"call_id_by_item": {}})
    ...
    try:
        ...
        if getattr(event, "type", None) == "raw_response_event":
            converted_event = _handle_raw_response(event.data, call_id_by_item)

        if getattr(event, "type", None) == "run_item_stream_event":
            converted_event = _handle_run_item_stream(event)
