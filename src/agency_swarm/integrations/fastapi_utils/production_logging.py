...
class ProductionLogger:
    ...
    def _track_error(self) -> None:
        ...
        if (self.last_error_time is None or 
            current_time - self.last_error_time > self.error_window_minutes * 60):
            self.error_count = 0
        ...
        if self.error_count >= self.error_threshold:
            self._send_alert(
                f"High error rate detected: {self.error_count} errors in "
                f"{self.error_window_minutes} minutes"
            )
