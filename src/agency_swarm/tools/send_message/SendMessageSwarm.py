...
class SendMessageSwarm(SendMessageBase):
    ...
    def run(self) -> str | Generator[MessageOutput, None, None]:
        ...
        try:
            ...
            logging.error(f"SendMessageSwarm.run: BadRequestError while submitting tool output: {e}")
            raise Exception(
        ...
        try:
            ...
            logging.debug(f"SendMessageSwarm.run: Changing recipient agent to {recipient_agent.name}")
