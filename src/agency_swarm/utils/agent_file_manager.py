...
class AgentFileManager:
    ...
    async def check_file_exists_and_upload(self, file_name_or_path: str) -> str | None:
        """
        ...
        Returns:
            File ID if file exists, None otherwise
        """
        # If no files folder is set, return None
        if not self.agent.files_folder or not self.files_folder_path:
            return None

        # Check if file exists
        files = self.list_files()
        file_names = self.get_file_names(files)

        # Check by exact name match
        for existing_file, file_id in zip(file_names, [f.id for f in files], strict=False):
            if existing_file == file_name_or_path:
                return file_id
        ...
        # File not found in vector store, try to upload it
        try:
            file_path = os.path.join(self.files_folder_path, file_name_or_path)
            if os.path.exists(file_path):
                return self.upload_file(file_path, include_in_vector_store=True)
        except Exception as e:
            logger.warning(f"Failed to upload file {file_name_or_path}: {e}")
