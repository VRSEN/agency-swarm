...
19. After non-trivial policy edits, reread the affected rules tree for distorted meaning, lost protections, duplicate rules, contradictions, and regressions. Use a fresh built-in review worker only when the mandate, risk, or public gate requires independent review; Codex CLI review is not a default policy gate.
