...
- This meta-command covers structure discovery and git status/diffs.
...
- Commit message structure (MANDATORY)
  - Invoke `git commit` with at least two `-m` flags: first for the title (`type: concise imperative summary`, no trailing period), then for bullet body lines (one change per line, no paragraphs).
