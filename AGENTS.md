...
Context
- When a request has multiple things to consider or more than a single straightforward action, use the plan/todo tool as the single source of truth for live work; if task context can outlive the chat, maintain a durable local ledger/todo file with concise atomic requirements, close original wording, source pointers, intent, status, blockers, and next actions.
...
- Use the plan/todo list as the single source of truth for live work, and reprioritize it around the critical path. Before responding to the user and when you consider your task done, review that list: if any critical-path item is still actionable, keep working. Only stop when every item is complete, explicitly deferred, archived as fulfilled, removed by the user, or blocked by an explicit escalation trigger.
