...
2.1 This Instruction File is the Agency Swarm supplement to `/Users/nick/.codex/AGENTS.md` and governs AI contributors to the repository.
...
2.10 Higher-priority instructions, `/Users/nick/.codex/AGENTS.md`, and User Requests control this file; if they conflict, surface the conflict and follow the higher rule.
...
3.2 Keep only repo-specific rules here, mirror `VRSEN/agentswarm-cli` shared policy as a strict subset or necessary Python/Agency adaptation, and never weaken `/Users/nick/.codex/AGENTS.md`.
...
3.14 For any non-trivial task, use `.codex/skills/delegation-management` and the smallest viable Native Subagent mandate.
...
5.3 Use `.codex/skills/requirement-ledger` on every turn as the sole durable task record.
5.4 Treat the Requirement Ledger as the only persistent system for status, future work, and artifact state.
5.5 Keep the Requirement Ledger in durable local files under `/Users/nick/.codex`, not in the repo. Do not treat chat memory as durable state.
...
## 9. Escalation
9.1 Use `.codex/skills/escalation-manager` and use Escalation only for design decisions or true blockers.
...
11.24 Use only the approval gates in this contract and `.codex/skills/test-workflow`. Do not invent slower gates.
...
11.33 Choose the smallest high-signal proof from `.codex/skills/test-workflow` that reduces uncertainty fastest.
...
17.2 General non-policy, non-release review may use `.codex/skills/codex-cli-review` and the General Review Command unless a repo skill or user Mandate requires a stronger path.
...
17.4 Policy, repo-skill, and workflow-rule edits shall use `.codex/skills/policy-maintenance` and the Policy Review Command through a separate isolated Codex Channel worker when available.
...
17.8 `.codex/skills/claude-cli-review` output and duplicate weaker runs may support high-reliability decisions but never replace the required Codex review path.
