...
## 🔴 API KEYS
- API keys (`OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, etc.) live in the workspace `.env` at the repo root. Load them before any live test: `set -a && source .env && set +a`.
- Pre-flight gate (real-LLM only): if planned validation includes integration tests/examples that call a real LLM, source the workspace `.env` and verify the required key is set before editing or running tests; if missing/invalid after sourcing, stop and report the blocker.
...
- Before asking the user for any key, source `.env` and inspect the environment to confirm it is actually missing or invalid.
