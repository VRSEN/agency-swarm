...
- Before any release or safety claim, launch the installed CLI against the maintainer's canonical local test agency, send a real first message through the connected conversation, and verify a non-empty streaming assistant response renders. Auth-smoke CI alone never satisfies this gate; any launch failure, including environment, credential, dependency, or TUI-transition issues, is a release blocker until reproduced and root-caused.
...
- On each user message, decide whether the Instruction File needs a policy adjustment so standing instructions from this chat stay clearly covered and repeated mistakes, user-visible failures, or recurring slowdown do not repeat; treat any user signal of negative performance, even a hint, especially `always` or `never`, as a policy trigger, and if the rule is missing or unclear, make a surgical update through Codex CLI with extra-high reasoning, grounded in the failure pattern instead of a one-off fix, such as unsanctioned releases, transcript leaks in PR bodies, conflict markers in live files, terminal screenshots treated as end-user proof, or forgotten authentication, without derailing the active critical path.
