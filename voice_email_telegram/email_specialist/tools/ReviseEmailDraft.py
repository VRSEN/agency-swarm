...
class ReviseEmailDraft(BaseTool):
    ...
    draft: str = Field(
        ...,
        description="JSON string containing the current draft (to, subject, body, metadata)"
    )

    feedback: str = Field(
        ...,
        description="User's feedback on what to change (can be voice transcription or text)"
    )

    chain_of_thought: str = Field(
        default="",
        description="Think step-by-step about which parts of the email need revision and how to best apply the feedback"
    )
...
Revise the email according to the feedback. Return only JSON with fields: to, subject, body"""
            ...
            return json.dumps({
                "error": f"Error revising draft: {str(e)}"
            })
...
if __name__ == "__main__":
    ...
    draft = json.dumps({
        ...
        "body": "Dear Sarah,\n\nI hope this email finds you well. I am writing to formally request a reorder of the blue widgets. We require 500 units at your earliest convenience.\n\nThank you for your attention to this matter.\n\nBest regards,\nUser"
    })
    tool = ReviseEmailDraft(
        draft=draft,
        feedback="Too formal, make it more casual and friendly"
    )
    result = tool.run()
    print(result)
    ...
    tool = ReviseEmailDraft(
        draft=draft,
        feedback="Add that it will arrive Tuesday instead of Monday and apologize"
    )
    result = tool.run()
    print(result)
    ...
    draft = json.dumps({
        ...
        "body": "Hello,\n\nWe're experiencing some issues with our server. Could you please look into it when you have a chance?\n\nThanks,\nUser"
    })
    tool = ReviseEmailDraft(
        draft=draft,
        feedback="This is urgent! Production is down, needs immediate attention"
    )
    result = tool.run()
    print(result)
    ...
    draft = json.dumps({
        ...
        "body": "Dear Team,\n\nI hope everyone is doing well. I wanted to take a moment to remind everyone about our upcoming meeting scheduled for tomorrow at 2 PM. It would be greatly appreciated if everyone could arrive on time and come prepared with any questions or updates they may have regarding their current projects. This will help us make the most efficient use of our time together.\n\nLooking forward to seeing everyone there.\n\nBest regards,\nUser"
    })
    tool = ReviseEmailDraft(
        draft=draft,
        feedback="Way too long, make it much shorter and to the point"
    )
    result = tool.run()
    print(result)
    ...
    tool = ReviseEmailDraft(
        draft=draft,
        feedback="Actually, send this to the whole team at team@company.com"
    )
    result = tool.run()
    print(result)
    ...
    draft = json.dumps({
        ...
        "metadata": {
            "revision_count": 2,
            "original_transcript": "Send proposal to client"
        }
    })
    tool = ReviseEmailDraft(
        draft=draft,
        feedback="Add that we're excited to work together"
    )
    result = tool.run()
    print(result)
