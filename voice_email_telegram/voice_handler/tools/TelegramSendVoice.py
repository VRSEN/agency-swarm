...
class TelegramSendVoice(BaseTool):
    ...
    chat_id: str = Field(
        ...,
        description="Telegram chat ID to send voice message to"
    )

    voice_file_path: str = Field(
        ...,
        description="Local path to the voice file (OGG or MP3 format)"
    )
    ...
    def run(self):
        ...
        if not os.path.exists(self.voice_file_path):
            return json.dumps({
                "error": f"Voice file not found at path: {self.voice_file_path}"
            })
        ...
        try:
            ...
            return json.dumps({
                "error": f"Failed to send voice message: {str(e)}"
            })
        except Exception as e:
            return json.dumps({
                "error": f"Error sending voice: {str(e)}"
            })
