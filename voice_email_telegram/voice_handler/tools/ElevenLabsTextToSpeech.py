...
class ElevenLabsTextToSpeech(BaseTool):
    ...
    text: str = Field(
        ...,
        description="Text to convert to speech (max 5000 characters for standard voices)"
    )
    ...
    def run(self):
        ...
        if len(self.text) > 5000:
            return json.dumps({
                "error": f"Text is too long ({len(self.text)} chars). Maximum is 5000 characters."
            })
        ...
        try:
            ...
            return json.dumps({
                "error": f"Failed to connect to ElevenLabs: {str(e)}"
            })
...
if __name__ == "__main__":
    ...
    tool = ElevenLabsTextToSpeech(
        text="Hello! I wanted to let you know that your email draft is ready for review. Please check your Telegram messages to approve or reject the draft. Thank you!",
        output_path="/tmp/voice_confirmations"
    )
    result = tool.run()
    print(result)
