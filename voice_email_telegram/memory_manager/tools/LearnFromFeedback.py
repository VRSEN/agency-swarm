...
class LearnFromFeedback(BaseTool):
    ...
    draft: str = Field(..., description="JSON string containing the email draft that was reviewed")

    feedback: str = Field(
        ..., description="User's feedback (can be empty for approvals, or revision instructions for rejections)"
    )

    action: str = Field(..., description="User action: 'approved' or 'rejected'")

    recipient: str = Field(default="", description="Email recipient (to identify recipient-specific preferences)")
    ...
    def run(self):
        ...
        try:
            ...
            if self.action not in ["approved", "rejected"]:
                return json.dumps({"error": f"Invalid action '{self.action}'. Must be 'approved' or 'rejected'"})
...
Return only JSON."""
                ...
                if not self.feedback or self.feedback.strip() == "":
                    return json.dumps(
                        {"learnings": [], "message": "No feedback provided with rejection. Cannot extract learnings."}
                    )
...
What should we learn from this rejection? Return only JSON."""
            ...
            return json.dumps({"error": f"Invalid JSON in draft: {str(e)}", "learnings": []})
        except Exception as e:
            return json.dumps({"error": f"Error analyzing feedback: {str(e)}", "learnings": []})
...
if __name__ == "__main__":
    ...
    draft = json.dumps(
        {
            ...
            "body": "Hi John,\n\nI wanted to reach out regarding your recent order. Unfortunately, we've experienced "
            "a slight delay in shipping. The order will now arrive on Tuesday instead of Monday as originally "
            "scheduled.\n\nWe apologize for any inconvenience this may cause and appreciate your "
            "understanding.\n\nBest regards,\nSarah Johnson",
        }
    )
    tool = LearnFromFeedback(draft=draft, feedback="", action="approved", recipient="john@acmecorp.com")
    result = tool.run()
    print(result)
    ...
    draft = json.dumps(
        {
            ...
            "body": "Dear Ms. Sarah,\n\nI hope this correspondence finds you in good health. I am writing to formally "
            "request a reorder of the blue widgets previously discussed.\n\nYours sincerely,\nUser",
        }
    )
    ...
    draft = json.dumps(
        {"to": "supplier@example.com", "subject": "Reorder", "body": "Hi,\n\nWe need to reorder.\n\nThanks"}
    )
    tool = LearnFromFeedback(
        draft=draft,
    ...
    tool = LearnFromFeedback(draft=draft, feedback="", action="approved", recipient="colleague@company.com")
    result = tool.run()
    print(result)
    ...
    draft = json.dumps(
        {
            ...
            "body": "Dear Valued Client,\n\nI hope this email finds you well and that you're having a wonderful week "
            "so far. I wanted to take this opportunity to reach out to you regarding the ongoing project that "
            "we've been collaborating on together.\n\nAs we continue to make progress, I felt it would be "
            "beneficial to provide you with a comprehensive update...\n\n[continues for several more paragraphs]",
        }
    )
    tool = LearnFromFeedback(
        draft=draft,
    ...
    draft = json.dumps({"to": "test@example.com", "subject": "Test", "body": "Test body"})
    tool = LearnFromFeedback(draft=draft, feedback="", action="rejected")
    result = tool.run()
    print(result)

    # Test 8: Invalid action
    print("\n8. Invalid action:")
    draft = json.dumps({"to": "test@example.com", "subject": "Test", "body": "Test"})
    tool = LearnFromFeedback(draft=draft, feedback="Some feedback", action="invalid_action")
    result = tool.run()
    print(result)
