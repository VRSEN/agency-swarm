...
class Mem0Add(BaseTool):
    ...
    text: str = Field(
        ...,
        description="The memory text to store (preference, pattern, or context information)"
    )

    user_id: str = Field(
        ...,
        description="User identifier to associate this memory with"
    )

    metadata: str = Field(
        default="{}",
        description="Optional JSON string with additional metadata (category, confidence, tags)"
    )
    ...
    def run(self):
        ...
        if not api_key:
            return json.dumps({
                "error": "MEM0_API_KEY not found in environment variables"
            })

        if not self.text or len(self.text.strip()) < 3:
            return json.dumps({
                "error": "Memory text is too short. Provide meaningful content to store."
            })
