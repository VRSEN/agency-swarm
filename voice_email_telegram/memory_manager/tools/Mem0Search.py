...
class Mem0Search(BaseTool):
    ...
    limit: int = Field(default=5, description="Maximum number of memories to return (1-20)")
    ...
    def run(self):
        ...
        if not self.query or len(self.query.strip()) < 2:
            return json.dumps({"error": "Search query is too short. Provide a meaningful search term."})
        ...
        try:
            ...
            payload = {"query": self.query, "user_id": self.user_id, "limit": min(max(self.limit, 1), 20)}
...
if __name__ == "__main__":
    ...
    tool = Mem0Search(query="Sarah supplier", user_id="user_12345", limit=5)
    result = tool.run()
    print(result)

    # Test 2: Search for signature preferences
    print("\n2. Search for signature:")
    tool = Mem0Search(query="how to sign emails", user_id="user_12345", limit=3)
    result = tool.run()
    print(result)

    # Test 3: Search for tone preferences
    print("\n3. Search for tone preferences:")
    tool = Mem0Search(query="email tone style", user_id="user_12345", limit=5)
    result = tool.run()
    print(result)

    # Test 4: Search for client preferences
    print("\n4. Search for client-specific preferences:")
    tool = Mem0Search(query="ABC Inc client", user_id="user_12345", limit=2)
    result = tool.run()
    print(result)

    # Test 5: Generic search
    print("\n5. Generic search:")
    tool = Mem0Search(query="email preferences", user_id="user_12345", limit=3)
    result = tool.run()
    print(result)

    # Test 6: Empty query
    print("\n6. Test with empty query:")
    tool = Mem0Search(query="", user_id="user_12345")
    result = tool.run()
    print(result)

    # Test 7: Different user
    print("\n7. Search for different user:")
    tool = Mem0Search(query="preferences", user_id="user_67890", limit=5)
    result = tool.run()
    print(result)
