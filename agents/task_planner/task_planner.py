...
# 注意: 除非用户请求中有提供初始条件的描述，初始环境中除了华为云账号和可用的华为云AK/SK，没有创建**任何资源**，且不提供任何**资源和环境信息**；确保你的规划有你任务中**所需资源的创建**或**所需信息的获取**步骤，否则请先完成它们；
# 注意: 为了防止用户隐私不被泄露，华为云认证信息已被执行任务的agent得知，你的任务规划中不需要获取华为云AK/SK等认证信息
