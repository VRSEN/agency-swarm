...
请注意，每个task你都需要仔细思考原始用户输入中与该任务相关的信息，并**详细地**写入"description"字段中，参数值等信息不能省略
...
如果需要重新规划，你需要从context_tree.json中读取已完成的任务以及之前任务完成过程中的上下文信息，从error.json中读取之前任务完成过程中的出现的error信息，一步步思考该在原先规划上如何修改，保证推进用户请求 (即总任务) 的完成
...
确保你的规划结果应该尽可能可以并行执行。如果两个任务可以同时开始执行而彼此不冲突，则可以并行执行。
