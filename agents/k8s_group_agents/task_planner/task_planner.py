...
# 规划开始之前，你需要判断用户输入请求是否只与文字输出有关（例如：输出复盘报告、输出预案、输出自动化脚本等），如果有关，你只需要将用户输入请求中的相关信息传递给监控能力群中的文本输出agent来进行输出（只有这一个task），不需要规划其他任何多余的task。
...
如果需要重新规划，你需要调用工具`ReadJsonFile`从completed_tasks.json中读取已完成的任务，从context.json中读取之前已完成的所有步骤的上下文信息，从error.json中读取之前任务完成过程中的出现的error信息，一步步思考该在原先规划上如何修改，保证推进用户请求 (即总任务) 的完成。
