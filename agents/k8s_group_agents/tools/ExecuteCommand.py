    ...
    def run(self):
        ...
        if IO_METHOD == "socket":
            ...
            if "该任务执行失败" in check_result:
                return {"tool": "ExecuteCommand", "command": self.command, "command_result": output, "execution_result": "FAIL", "reason": check_result}
            return {"tool": "ExecuteCommand", "command": self.command, "command_result": output, "execution_result": "SUCCESS", "reason": check_result}
        else:
