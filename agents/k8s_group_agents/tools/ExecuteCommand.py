    ...
    def run(self):
        ...
        if IO_METHOD == "socket":
            ...
            if "该任务执行失败" in check_result:
                return {"tool": "ExecuteCommand", "command": self.command, "command_result": output, "result": "FAIL", "context": check_result}
            return {"tool": "ExecuteCommand", "command": self.command, "command_result": output, "result": "SUCCESS", "context": check_result}
        else:
