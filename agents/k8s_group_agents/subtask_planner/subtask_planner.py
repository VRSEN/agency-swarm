...
同时，你需要先通过`ReadJsonFile`从completed_sub_tasks.json读取已完成的子任务，从context_index.json中读取已完成任务产生的环境信息。（直接调用工具，不要把它规划为一个subtask）

获得以上信息后，你需要严谨专业地一步步思考，根据任务描述对该任务进行拆分。你需要确保:
