...
同时，你需要先调用工具`ReadJsonFile`从context_tree.json中读取上下文信息（直接调用工具，不要把它规划为一个subtask）。其中，"status"为"completed"表示已经完成，"status"为"executing"表示当前正在执行，"status"为"pending"表示还未执行。
获取以上信息后，你需要判断用户输入请求是否与**之前已完成的过程**有关，如果有关，从上下文信息中提取有用信息，并结合该信息进行后续的task规划。
...
3. subtask的内容**不能与其他任何任务的内容有重复**，避免过度规划。
