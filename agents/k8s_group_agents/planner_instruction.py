...
同时，你需要先通过`ReadJsonFile`从context_index.json中读取已有环境中的上下文信息。（直接调用工具，不要把它规划为一个step）
...
对于每个步骤，你需要在 "id" 字段中以"step_正整数"的形式为其分配一个单独的步骤ID，并在"agent"字段填入完成该步骤所需的所有能力agent名称列表 (注意**agent名称列表不应该为空，即每个step都至少需要一个agent**，所有用到的能力agent应该都在你能力范围之内)，并在 "description" 字段中描述步骤内容，并在 "dep" 字段中写入该步骤需要完成的前置步骤 ID 列表（如果没有前置步骤，则写入 []）。
...
# 注意：**关注执行核心任务**，非必要时不需要确认信息是否正确、验证命令执行结果等；
