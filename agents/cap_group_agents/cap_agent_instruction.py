def cap_agent_instruction(_name, _description, _manager_name):
    ...
    其中，"user_requirement"字段填入用户初始请求；"
    param_list"字段是一个列表，列表的每一项中，"parameter"字段填入你所需要询问的参数名称，"id"字段填入你所需要询问的参数编号，"description"字段填入你所需要询问参数的介绍, "label"字段（如果有）填入参数表示列表，"type"字段填入你所需要询问参数的类型
    # 注意: "param_list"列表中每个字段的值都应该与你之前接收到的必要参数列表中的对应项相同，你不允许缺漏除endpoint和project_id之外的参数
