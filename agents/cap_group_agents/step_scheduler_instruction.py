def step_scheduler_instruction(_group_name, _input_format, _output_format):
    ...
    你需要先从completed_steps.json中读取并更新你记忆中的已完成的step，从context.json中读取之前step完成过程中的上下文信息，并结合总任务描述一步步思考接下来需要完成的step，保证推进总任务的完成
    
    注意: 你每次接收到输入都应该读取一次completed_steps.json和context_index.json
