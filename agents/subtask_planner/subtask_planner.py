...
现有的能力群名称和介绍如下: 
"统一身份认证服务IAM能力群": 统一身份认证服务IAM能力群提供智能体账户信息管理能力，例如获取访问密钥（AK）和私密密钥（SK），用于身份验证和授权；
"操作系统管理能力群": 该操作系统管理能力群提供通过SSH远程连接ECS执行命令的能力；
"华为云API处理能力群": 华为云API处理能力群提供了一种访问和使用华为云API的通用方法。它能够根据API描述检索API信息，组装API请求，并调用API执行操作；
...
对于每个子任务，你需要在 "id" 字段中为其分配一个单独的子任务ID，并在"capability_group"字段填入完成该子任务所需的能力群名称，并在 "description" 字段中描述任务内容，并在 "dep" 字段中写入该子任务需要完成的前置子任务 ID 列表（如果没有前置任务，则写入 []），允许环的构建，表示这些子任务需要多次迭代执行。
