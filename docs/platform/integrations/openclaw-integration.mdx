...
- An Agencii account
- A GitHub account
- Model credentials (provider API key, or OpenClaw OAuth login access)
...
<Steps>
  ...
  <Step title="Add model credentials">
    If you use API-key based models, add provider API keys in the Agencii key modal.
    If you use the default Codex OAuth model, you will run one shell command in the next step.
  ...
  <Step title="Run one-time OpenClaw OAuth bootstrap">
    The starter template defaults to `openai-codex/gpt-5.3-codex`, which needs one setup command in your deployment shell:

    ```bash
    openclaw models auth login --provider openai-codex
    ```

    Complete sign-in in your browser, then paste the redirect URL/code back into the shell prompt.
  </Step>
...
## One-Time OAuth Bootstrap (Operator Step)

This command uses the OpenClaw CLI in your deployment shell. Your end users do not run CLI commands.

```bash
openclaw models auth login --provider openai-codex
```

After login, OpenClaw writes the auth profile into persistent state under `/mnt/openclaw/state`.
You can verify with:
...
Use this only if you want to test on your own machine before deploying.

<Accordion title="Show local setup steps">
...
3. Add your model credentials and app token:
   - `OPENAI_API_KEY=...` (or another provider key), or run Codex OAuth bootstrap after startup
   - `APP_TOKEN=local-openclaw-token`
   - Ensure Node `>=22.12.0` is available
   - If you have multiple Node installs, set `OPENCLAW_NODE_BIN` to a compatible binary (for example `/opt/homebrew/bin/node`)
...
OAuth auth profiles are also stored in that same persistent runtime (`/mnt/openclaw/state`).
