...
<Steps>
  <Step title="Create a new widget">
    ...
    4. Select Agency and Selectable Agents.
    5. Select any advanced settings like Allowed Domains, open widget automatically, etc.
    6. Save changes.
    7. Test your widget!
  </Step>
  ...
  <Step title="Embed the widget">
    ...
    2. Click on the 3 dot menu on the right side of the widget row and select **Embed**.
  ...
  <Step title="Share the widget">
    1. In the Agencii dashboard, open **Deploy → Widgets** and find the widget you created.
    2. Click on the 3 dot menu on the right side of the widget row and select **Share** OR use Share button near the 3 dot menu.
    3. It will open a new window with the widget embedded into simple blank page.
...
<Tip>
You can embed several widgets on the same page by using embedding script multiple times with different `widgetId`s and `chatContainer` selectors.
</Tip>
...
### 2) Send Message to Widget

Programmatically sends a text message into the widget (e.g., seed a greeting or pass context).
This event does not open the widget automatically. So, dispatch `open-widget` event first.
