...
## Next.js + Map (recommended)

If your app is Next.js, embed the widget on the **client side** (App Router: a `use client` component) and render your map as normal.

The simplest pattern is:

- Render your map UI (Leaflet, Mapbox, Google Maps, etc.)
- Paste the widget embed script into the same page/component
- Build `additionalInstructions` from the selected coordinates and pass it as the **third argument** to `widget.init(...)`

<Note>
Do not test this from `file://...`. Run your Next.js dev server so your page has a real origin (like `http://localhost:3000`), and allow that origin in the widget settings if you use Allowed Domains.
</Note>
