...
<Steps>
  ...
  <Step title="Enable persistent storage">
    ...
    OpenClaw uses `/mnt/openclaw` as its runtime root.
    In Agent Swarm's file browser, the same mounted volume appears under `/app/mnt/openclaw`.
  </Step>
...
Its runtime path is `/mnt/openclaw/.openclaw/workspace`.
...
![Agent Swarm persistent file storage view for OpenClaw workspace](/images/platform/openclaw-persistent-storage.png)
...
- Persistent OpenClaw files under `/mnt/openclaw`, visible in the file browser at `/app/mnt/openclaw`
