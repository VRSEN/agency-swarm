...
<Steps>
  ...
  <Step title="Enable persistent storage">
    ...
    OpenClaw keeps its state and workspace files under `/mnt/openclaw`, so this needs to be enabled.
  </Step>
...
On Agent Swarm, the full path is `/mnt/openclaw/.openclaw/workspace`.
