...
<Steps>
  ...
  <Step title="Add your AI provider secrets">
    ...
    Only add extra settings if you are intentionally changing the default model or provider.
  </Step>
...
<Note>
The onboarding form is not the place for a long system prompt. Start with the minimum required text, then move the real customization into the workspace files after deploy.
</Note>
