...
def terminal_demo(self, show_reasoning: bool | None = None, reload: bool = True) -> None:
    """
    Run a terminal demo of the agency (set reload=False to disable hot reload on file changes).
    """
```
...
class Agent(BaseAgent[MasterContext]):
    def __init__(self, **kwargs: Any):
        ...
        Parameters:
            ...
            raise_input_guardrail_error (bool): Controls input guardrail behavior—False enables non-strict mode (guidance as assistant message), True enables strict mode (raises exceptions). Alias: `throw_input_guardrail_error`. Default: False
...
- **`raise_input_guardrail_error`** (bool): Controls input guardrail mode—False for non-strict (guidance as assistant), True for strict (raises exceptions). Alias: `throw_input_guardrail_error`
