        ...
        *entry_point_agents: Agent,
        ...
        Parameters:
            *entry_point_agents: Agent instances serving as entry points for external interaction
            ...
            memory: Durable memory configuration for advanced setups
            memory_folder: Simple markdown-backed durable memory root folder
        """
```
...
async def get_response(
    ...
    **kwargs: Any,
    ...
    Parameters:
        ...
        memory_identity: Advanced durable memory identity override
        user_id: Convenience durable-memory user scope for this run
        session_id: Convenience durable-memory session scope for this run
...
def run_fastapi(
    ...
    Parameters:
        ...
        allow_client_memory_identity: Trust raw durable-memory identity from request bodies
        memory_identity_resolver: Bind trusted durable-memory identity on the server
    """
```
...
For provisioning and current terminal limitations, see [Agent Swarm CLI](/core-framework/agencies/agent-swarm-cli).
