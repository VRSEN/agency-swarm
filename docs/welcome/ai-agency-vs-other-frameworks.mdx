...
| **Reliability**                    | ✅ Robust type checking and validation for all tools with Pydantic. Input and output validation with agent's [guardrails system](/additional-features/guardrails)   | ❌ Type hints but no validation.                                                                                        | ❌ Some validation is possible when using BaseTool, although the interface is not convenient to use.                       |
