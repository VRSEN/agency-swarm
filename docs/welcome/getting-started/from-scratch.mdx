...
<Steps>
  ...
  <Step title="Create Tools">
    Define your custom tools by extending the `BaseTool` class and implementing
    the `run` method. Starting with **v1.x** you can also create plain functions
  ...
  <Step title="Define Agent Roles">
    ...
    **Developer.py:**
    ...
    Tools will be imported automatically from the `tools` folder. Each
    tool file may define a class that inherits from `BaseTool` or a
