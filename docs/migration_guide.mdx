...
| v0.x Method | v1.x Method (Recommended) | v1.x Backward Compatible |
|-------------|---------------------------|--------------------------|
| `agency_chart` parameter | Positional arguments for entry points + `communication_flows` parameter | `agency_chart` (deprecated) |
| `threads_callbacks` | `load_threads_callback` + `save_threads_callback` | - |
| `get_completion()` | `get_response()` (async) | `get_completion()` (sync, deprecated) |
| `get_completion_stream()` | `get_response_stream()` (async) | `get_completion_stream()` (sync, deprecated) |
...
<Note>
**Backward Compatibility**: `get_completion()` and `get_completion_stream()` remain synchronous in v1.1 for backward compatibility. While deprecated, they work exactly as in v0.x to minimize migration friction.
</Note>
</Step>
