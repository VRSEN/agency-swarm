...
`agentswarm-cli` is the npm package that provides the Agent Swarm CLI terminal app used by `agency-swarm`. The preview below shows the current `tui()` startup screen.
...
`tui()` downloads the matching terminal binary on first run, caches it locally, starts the local `agency-swarm` bridge, and opens the TUI.
