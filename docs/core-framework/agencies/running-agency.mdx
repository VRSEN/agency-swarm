...
To run the agency directly from your terminal, use the `tui` method:

```python
agency.tui()
```

On first run, `agency-swarm` downloads the matching `agentswarm-cli` package automatically, caches the terminal app locally, starts the local bridge, and opens the terminal UI.
