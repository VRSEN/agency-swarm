...
1) Register an OAuth app with your provider and set the redirect URI to `http://localhost:3000/callback`.
...
- GitHub: Developer Settings → **New OAuth App**, set callback to `http://localhost:3000/callback`, copy Client ID/Secret.
- Google: Cloud Console → Credentials → **OAuth client ID** (Desktop), set callback to `http://localhost:3000/callback`, copy Client ID/Secret.
...
- Redirects: keep `http://localhost:3000/callback` for desktop; override `redirect_uri` for hosted flows.
...
async def send_redirect(auth_url: str) -> None:
    emit_sse("oauth_required", {"url": auth_url})
