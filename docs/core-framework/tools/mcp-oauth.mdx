...
- **HostedMCPTool (remote MCP)**: If you use `HostedMCPTool`, OAuth tokens must be provided via `tool_config.authorization`. In FastAPI, wrap only the OAuth-protected hosted tool with `enable_hosted_mcp_tool_oauth(...)`. Agency Swarm will create an in-memory registry by default, or you can pass `oauth_registry=OAuthStateRegistry()` to share state across workers. It then surfaces the auth URL via `event: oauth_redirect` and injects the access token once authorized. Public hosted MCP servers should stay unwrapped.
...
Tokens default to `~/.agency-swarm/mcp-tokens/{user}/{server-endpoint}/`. Set `AGENCY_SWARM_MCP_CACHE_DIR` for a global override or pass `cache_dir` per server.

User-scoped buckets now use a collision-resistant filesystem segment derived from `X-User-Id` or `set_oauth_user_id()`. Legacy buckets whose old sanitized name could map to more than one user ID are intentionally not auto-migrated; those users must authorize again instead of risking cross-user token reuse.

For per-user isolation in FastAPI, pass the `X-User-Id` header (the integration sets the OAuth user context automatically). For local or other non-FastAPI runs, either provide `oauth_token_path` with `user_context`, or call `set_oauth_user_id()` before the run:
...
```
./data/oauth-tokens/
  default/github-<endpoint-hash>/tokens.json
  user_123-<hash>/github-<endpoint-hash>/tokens.json
```
