...
- **Deferred auth trigger:** SaaS mode does not force OAuth discovery on the first user message. The model gets an `activate_mcp_tools` tool and OAuth starts only when that tool is invoked.
