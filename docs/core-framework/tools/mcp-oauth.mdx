...
OAuth authentication for MCP servers unlocks access to services that require user authorization:

- **Personal Data Access**: Let agents read and write to your Notion workspace, GitHub repositories, or Google Drive files
- **Secure Authorization**: Users control exactly what data agents can access through OAuth permission scopes
- **Persistent Sessions**: Agents remember authorization across runs—no need to re-authenticate every time
- **Multi-Service Integration**: Connect to multiple OAuth-protected services from a single agent
...
2. **MCP Server**: An MCP server that supports OAuth (see [FastMCP](https://fastmcp.cloud/) for examples with built-in OAuth providers)
...
<ParamField param="storage" type="TokenStorage">
  Custom token storage implementation. If not provided, uses file-based storage (for local development). For SaaS deployments, pass a custom storage backend that implements the MCP SDK's TokenStorage interface.
</ParamField>
...
<Tabs>
<Tab title="Local Development (V0)">
For local development and single-user scenarios, Agency Swarm uses file-based token storage:

- Tokens are saved automatically after authorization
- Secure file permissions protect token data
- No configuration required—works out of the box

This is the default behavior and suitable for most development workflows.
</Tab>

<Tab title="SaaS Deployment (V1)">
For multi-tenant SaaS deployments, provide a custom storage implementation:

```python
from agency_swarm.mcp import MCPServerOAuth

# Custom storage implementing MCP SDK's TokenStorage interface
custom_storage = DatabaseTokenStorage(
...
oauth_server = MCPServerOAuth(
    url="http://localhost:8001/mcp",
    name="github",
    storage=custom_storage  # Use database instead of files
)
```

Your custom storage backend receives callbacks for loading and saving tokens, enabling:
- Per-user token isolation in database
