...
<Note>
FileTokenStorage saves tokens under `~/.agency-swarm/mcp-tokens` by default. Set `AGENCY_SWARM_MCP_CACHE_DIR` or pass `cache_dir=Path("./data/oauth-tokens")` to `MCPServerOAuth` when you need a different directory or multiple token caches per user/account.
</Note>
