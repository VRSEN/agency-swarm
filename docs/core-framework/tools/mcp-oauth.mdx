...
**SaaS Token Persistence**: Need to stash tokens in Firestore or another secret store? Wire up `OAuthTokenHooks(load_tokens_callback=..., save_tokens_callback=...)` or pass a custom `TokenStorage`. The default storage now calls those callbacks automatically so you can sync tokens without touching disk.
</Note>
