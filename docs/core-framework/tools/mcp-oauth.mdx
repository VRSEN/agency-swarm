...
For a runnable script, see [`examples/mcp_oauth/notion_client.py`](https://github.com/VRSEN/agency-swarm/blob/main/examples/mcp_oauth/notion_client.py).
...
<ParamField param="callback_handler" type="Callable[[], Awaitable[AuthorizationCodeResult]]">
  Custom async handler to receive the OAuth callback. Returns an `AuthorizationCodeResult` from `mcp.shared.auth` with `code`, `state`, and `iss` fields. By default, starts a local HTTP server (or prompts for manual paste) to capture the redirect. In FastAPI deployments you typically do **not** set this—`run_fastapi` wires callback handling through `/auth/callback`.
</ParamField>
...
Do not read the user ID directly from a request header or body. If browsers reach your app with a cookie or session, also pass `verify_oauth_callback_user=True` so `/auth/callback` rejects a code whose pending flow belongs to a different user. See [Binding the callback to a user](/additional-features/fastapi-integration#binding-the-callback-to-a-user).

For local or other non-FastAPI runs, either provide `oauth_token_path` and pass `user_id` through `context_override`, or call `set_oauth_user_id()` before the run:
...
response = await agency.get_response(
    ...
    context_override={"user_id": "user_123"},
)
```
...
```
./data/oauth-tokens/
  default/github-<endpoint-and-client-hash>/tokens.json
  user_123-<hash>/github-<endpoint-and-client-hash>/tokens.json
```
...
async def wait_for_code() -> AuthorizationCodeResult:
    code, state, iss = await poll_frontend()
