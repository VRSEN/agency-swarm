...
Start with this quick mental model:

<Steps>
<Step title="1. User prompt">
The agent receives a prompt that requires data behind OAuth.
</Step>
<Step title="2. Token check">
The agent reads cached tokens from its token cache (FileTokenStorage).
</Step>
<Step title="3. Browser hop (only if needed)">
If no valid token exists, the agent launches the OAuth URL so the user can approve access.
</Step>
<Step title="4. Tool call">
With a valid token in place, the agent invokes the MCP tool and returns the secured data.
</Step>
</Steps>
...
sequenceDiagram
    ...
    User->>Agent: User prompt (needs OAuth data)
    Agent->>Agent: Read tokens from token cache
    alt Tokens missing or expired
        Agent->>Browser: Launch authorization URL
        Browser->>OAuth: User signs in and approves scopes
        OAuth-->>Browser: Redirect with code/state
        Browser->>Agent: Send callback URL/code
        Agent->>OAuth: Exchange code for access/refresh tokens
        Agent->>Agent: Save tokens via FileTokenStorage
    end
    Agent->>MCP: Invoke OAuth-enabled MCP tool
    MCP-->>Agent: Return tool result
    Agent-->>User: Respond with secured data
```

<Note>
FileTokenStorage saves tokens under `~/.agency-swarm/mcp-tokens` by default. Set `AGENCY_SWARM_MCP_CACHE_DIR` or pass `cache_dir=Path("./data/oauth-tokens")` to `MCPServerOAuth` when you need a different directory or multiple token caches per user/account. Within each cache dir, tokens live in `<user>/<server>/tokens.json` along with `client.json`.
</Note>
...
Tokens live under `~/.agency-swarm/mcp-tokens` unless you override the cache directory. Override globally via `AGENCY_SWARM_MCP_CACHE_DIR`, or per server with `cache_dir`. For SaaS flows, pair `OAuthStorageHooks` with `user_context={"user_id": ...}` so each run writes into its own subfolder:
...
oauth_server = MCPServerOAuth(
    ...
    name="github",
    scopes=["repo", "user"],
    cache_dir=Path("./data/oauth-tokens"),  # Tokens saved here
)
...
agency = Agency(
    [agent],
    hooks=[OAuthStorageHooks()],           # Enables per-user isolation
    user_context={"user_id": "user_123"},  # Optional for SaaS
)
```

Skip the hook and `user_id` for single-user/local flows—the storage falls back to the `default/` directory automatically.
...
<Step title="Export Server Credentials">
In **Terminal 1** (where you will run the MCP server), set the GitHub credentials so FastMCP can complete the upstream OAuth flow:
```bash
...
Open **Terminal 2** (no GitHub env vars required) and run:
```bash
...
4. Terminal prompts: `Paste the callback URL:` if automatic capture fails
...
- **Client Not Registered (browser error)**: Delete the local cache files (default `~/.agency-swarm/mcp-tokens/default/oauth-test-server_*`) or your custom `cache_dir`, then rerun the client so it re-registers automatically.
- **Token Cache Location**: Set `AGENCY_SWARM_MCP_CACHE_DIR` or configure `MCPServerOAuth(cache_dir=Path("./data/oauth-tokens"))` if you prefer storing tokens inside your repo.
- **Callback Issues**: Ensure the redirect URL exactly matches the GitHub OAuth app (`http://localhost:3000/callback`). If the auto listener fails, paste the full callback URL when prompted.
</Accordion>
