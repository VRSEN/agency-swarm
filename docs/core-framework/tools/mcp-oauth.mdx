...
User-scoped buckets use a collision-resistant filesystem segment derived from the authenticated user ID or `set_oauth_user_id()`. Legacy buckets whose old sanitized name could map to more than one user ID are intentionally not auto-migrated; those users must authorize again instead of risking cross-user token reuse.
...
Do not read the user ID directly from a request header or body. For local or other non-FastAPI runs, either provide `oauth_token_path` with `user_context`, or call `set_oauth_user_id()` before the run:
