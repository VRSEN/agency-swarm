...
- **Deferred auth trigger:** SaaS mode does not force OAuth discovery on the first user message. The model gets an `authenticate_mcp_server(server_name)` tool and OAuth starts only when that tool is invoked for a specific authenticated MCP server.
