...
<Steps>
  ...
  <Step title="Add the Tool to an Agent">
    ...
    <Accordion title="Using tools folder" icon="folder">
      ...
      <Note>
        Each file in the `tools_folder` should contain a class that is named
        exactly the same as the file name. For example, `Calculator.py` should
        contain a `Calculator` class. Alternatively, a file may define a
        function decorated with `@function_tool`. Both styles are loaded
        automatically, and class-based tools must inherit from `BaseTool`.
        If the path provided to `tools_folder` is not absolute, it will be
        resolved relative to the file where the agent class is defined.
      </Note>
    </Accordion>
  </Step>
</Steps>
