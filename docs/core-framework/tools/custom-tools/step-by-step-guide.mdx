...
<Steps>
  ...
  <Step title="Add the Tool to an Agent">
    ...
    <Accordion title="Using tools folder" icon="folder">
      ...
      <Note>
        Each file in the `tools_folder` can contain either a class-based tool
        named the same as the file or a function decorated with
        `@function_tool`. Both styles are discovered automatically.
      </Note>
    </Accordion>
  </Step>
</Steps>
