...
<Steps>
  ...
  <Step title="Add the Tool to an Agent">
    ...
    <Accordion title="Using tools folder" icon="folder">
      ...
      <Note>
        Each file in the `tools_folder` may define a class-based tool that
        inherits from `BaseTool` (the v0.x style) or a function decorated with
        `@function_tool`. Both styles are loaded automatically.
      </Note>
    </Accordion>
  </Step>
</Steps>
