...
<Steps>
  ...
  <Step title="Add the Tool to an Agent">
    ...
    <Accordion title="Using tools folder" icon="folder">
      ...
      <Note>
        Each file in the `tools_folder` may define a tool class that inherits
        from `BaseTool`&nbsp;— the pattern used in v0.x&nbsp;— or a function
        decorated with `@function_tool`. Both styles are loaded automatically.
      </Note>
    </Accordion>
  </Step>
</Steps>
