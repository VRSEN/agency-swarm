...
<CardGroup cols={2}>
  ...
  <Card title="Data Analytics" icon="chart-area">
    Export large datasets as CSV, generate visual graphs, and let agents provide insights based on visualizations without overloading the context window.
  </Card>
</CardGroup>
...
class FetchRemoteArchive(BaseTool):
    """Return a remote ZIP using the helper."""
    archive_url: str = Field(default="https://example.com/archive.zip", description="Archive to expose")
