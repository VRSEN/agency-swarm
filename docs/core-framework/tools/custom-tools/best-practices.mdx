...
### Use Agency Context to Control the Tool Flow

Use `Agency Context` to validate previous actions taken by this or other agents, before allowing it to proceed with the next action.
...
@function_tool
async def action_2(ctx: RunContextWrapper[MasterContext], input_data: str) -> str:
    """Performs action 2, but only if action 1 was successful."""
    # Access the agency context
    context: MasterContext = ctx.context

    # Check if previous action was successful
    if context.get("action_1_result") == "failure":
        raise ValueError("Please proceed with the action_1 tool first.")

    # Perform the action and store result
    context.set("action_2_result", "success")
    return "Success. Action 2 has been completed."
```
