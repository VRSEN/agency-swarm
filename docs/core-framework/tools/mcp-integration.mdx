...
For a practical, runnable example using both `MCPServerStdio` and `MCPServerSse`, run [`examples/mcp_servers.py`](https://github.com/VRSEN/agency-swarm/blob/main/examples/mcp_servers.py).

*   **Local flow:** Start `python examples/utils/stdio_mcp_server.py` and `python examples/utils/sse_mcp_server.py` in separate terminals before running the script.
*   **Hosted flow:** Update the `HostedMCPTool`'s `server_url` to your ngrok tunnel (for example, `https://<subdomain>.ngrok-free.app/sse/`) and run `ngrok http http://localhost:8000` so the hosted demo can reach the SSE server.
*   Comment out `public_mcp_server_example()` if you only want to exercise the local stdio workflow.
