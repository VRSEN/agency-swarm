...
<Accordion title="MCPServerStreamableHttp: For HTTP Streaming Tools" icon="stream">
...
streaming_tool = MCPToolParams(
    name="stream_large_content",
    description="Process and stream large content with progress updates",
    schema={
        "type": "object",
        "properties": {
            "content": {
...
streamable_server = MCPServerStreamableHttp(
    ...
    pre_loaded_tools=[pre_loaded_tool]
)
```
</Accordion>
