...
Mount that proxy first with `attach_openclaw_to_fastapi(app)`.
...
Use this when OpenClaw is managed outside the Agency Swarm app.
If that server is meant to be a delegated worker, configure that OpenClaw runtime with worker-safe tool settings too.
  </Tab>
</Tabs>
...
<Accordion title="Host the OpenClaw proxy in the same FastAPI app">
  ...
  def create_agency(load_threads_callback=None):
      ...
      return Agency(openclaw_worker, load_threads_callback=load_threads_callback)
