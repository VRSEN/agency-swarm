...
Cache makes the first reply instant without calling the LLM.
...
- The FastAPI `/get_metadata` response exposes starters as `conversationStarters` and extra cache-only prompts as `quickReplies`.
- With `cache_conversation_starters=True`, either a starter or a quick reply can return a saved reply without calling the LLM.
<Info>
