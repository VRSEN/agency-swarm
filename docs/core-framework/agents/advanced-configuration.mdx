...
A plain string or callable is added on every model call of each top-level user turn. It appears only in the model input and is **not saved** in the thread history.
...
<Note>
The reminder boundary lives on `agency_swarm.Runner` — import `Runner` from `agency_swarm`, not `agents`, when you run an agent directly. It keeps reminders out of saved history just like an `Agency` does. Calling `agents.Runner` directly gives you stock SDK behavior with no reminder handling.
</Note>

If you save a run state to resume an interrupted run later, serialize it with `run_state_to_json` and restore it with `run_state_from_json`:

```python
from agency_swarm import Runner, run_state_from_json, run_state_to_json

result = await Runner.run(agent, "next")
state_json = run_state_to_json(result.to_state())

restored = await run_state_from_json(agent, state_json)
result = await Runner.run(agent, restored)
```

Use these instead of the SDK's `RunState.to_json()` / `RunState.from_json()`. The plain SDK methods drop the reminder state, so reminder counts restart on resume.
