...
When an agent has `WebSearchTool`, Agency Swarm now includes full checked source URLs by default. It does this by adding `web_search_call.action.sources` to `model_settings.response_include`.
...
Set `include_web_search_sources=False` if you do not want this include field added automatically.
