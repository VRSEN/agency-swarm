...
| Starter Cache *(optional)* | `cache_conversation_starters` | If `True`, use cached conversation starter runs from `AGENCY_SWARM_CHATS_DIR` (defaults to `.agency_swarm`). Default: `False` |
