...
| Tools Folder *(optional)* | `tools_folder` | Path to a directory containing tool definitions. Tools are automatically discovered and loaded from this directory. Supports both legacy BaseTool subclasses and modern FunctionTool instances. Default: `None` |
