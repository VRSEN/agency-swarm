...
| Raise Input Guardrail Error *(optional)* | `raise_input_guardrail_error` | If set to `True`, input guardrail errors raise an exception. If set to `False`, the guardrail message is returned as the agent's response. Alias: `throw_input_guardrail_error`. Default: `False` |
...
| Starter Cache *(optional)* | `cache_conversation_starters` | If `True`, speed up first responses by replaying cached starter replies. See [Advanced Configuration](/core-framework/agents/advanced-configuration#conversation-starters-cache). Default: `False` |
