...
| Memory *(optional)* | `memory` | Enables durable memory for this agent when the owning agency has memory configured. Use `True` for the default behavior or pass `AgentMemoryConfig(...)` for finer control. Default: `None`. |
