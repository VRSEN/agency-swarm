...
| Include Web Search Sources *(optional)* | `include_web_search_sources` | Automatically includes full WebSearchTool source URLs by adding `web_search_call.action.sources` to `model_settings.response_include`. Default: `True` |
