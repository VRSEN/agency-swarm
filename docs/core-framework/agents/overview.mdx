...
| Throw Input Guardrail Error *(optional)* | `throw_input_guardrail_error` | Controls input guardrail behavior: `False` enables **friendly mode** (default) where guidance is returned as an assistant message, while `True` enables **strict mode** where guardrail failures raise exceptions. Default: `False` |
