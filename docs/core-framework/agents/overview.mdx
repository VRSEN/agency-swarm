...
| System Reminders *(optional)* | `system_reminders` | Transient reminders shown to the model at supported triggers. A string runs before the first model call of each top-level user turn; use `EveryNToolCalls(...)` for within-run tool checkpoints. Default: `None` |
