...
| Tools Folder *(optional)* | `tools_folder` | Path to a directory containing tool files. Each file can hold a class that inherits from `BaseTool` or a function decorated with `@function_tool`. Default: `None` |
