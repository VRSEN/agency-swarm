...
| Quick Replies *(optional)* | `quick_replies` | List of `{prompt, response}` pairs. If a user message matches a prompt exactly (trimmed, case-insensitive), the agent returns the response without calling the model. Default: `None` |
