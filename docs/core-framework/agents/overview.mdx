...
| Quick Replies *(optional)* | `quick_replies` | Additional first-message prompts eligible for starter-cache replay without being shown as conversation starters. Each value must be a non-empty string. Default: `None` |
| Starter Cache *(optional)* | `cache_conversation_starters` | If `True`, speed up first responses by replaying cached replies for both `conversation_starters` and `quick_replies`. See [Advanced Configuration](/core-framework/agents/advanced-configuration#conversation-starters-cache). Default: `False` |
