...
When you add `WebSearchTool`, Agency Swarm includes full checked source URLs by default. It does this by setting `web_search_call.action.sources` under the hood.
...
If you want to disable this behavior, set `include_web_search_sources=False` on the agent.
