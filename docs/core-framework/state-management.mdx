...
Since in the Agents SDK history management is transferred to the end user, the agency-swarm framework introduced a thread management system 
that utilizes the SDK's `PersistenceHooks` and internal `ThreadManager` classes to handle, automate and simplify history management.  
All agent outputs will be processed and stored locally to allow for seamless agency usage within a single session.
To maintain persistence across multiple sessions, it also provides convenient callback methods which require minimal input to set up.
