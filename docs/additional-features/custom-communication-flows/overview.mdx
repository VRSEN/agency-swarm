...
<Note>
**`SendMessageSwarm` is Deprecated**: `SendMessageSwarm` will be removed in a future version. For sequential flows or transferring control between agents in one direction, use the `SendMessageHandoff` tool class.

Detailed guidance on implementing handoffs is available in the [Hybrid Patterns Documentation](/additional-features/custom-communication-flows/hybrid-patterns).
</Note>
...
- **Configuration**: Place a `SendMessageHandoff` tool in your agent's tools folder or set `send_message_tool_class=SendMessageHandoff` when creating the agent. The library will automatically detect this and configure handoffs using the OpenAI SDK internally.
