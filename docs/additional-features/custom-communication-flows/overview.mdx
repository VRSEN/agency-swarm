...
- **How it Works**: From the perspective of the underlying language model, a handoff is treated like a special tool (e.g., `transfer_to_BillingAgent`). To enable handoffs, you configure an agent with the `send_message_tool_class=SendMessageHandoff` parameter. The agent's description will be used as the handoff tool instructions. See [`SendMessageHandoff.create_handoff()`](https://github.com/VRSEN/agency-swarm/blob/main/src/agency_swarm/tools/send_message.py) for implementation details.
...
You can customize the reminder message or disable it entirely by defining a custom Handoff class as follows (see [`add_reminder` and `reminder_override` fields](https://github.com/VRSEN/agency-swarm/blob/main/src/agency_swarm/tools/send_message.py)):

```python
from agency_swarm.tools.send_message import SendMessageHandoff

class CustomHandoff(SendMessageHandoff):
    add_reminder = True # Set to False to completely remove the extra message
    reminder_override = "My custom reminder" # Define custom reminder message
```
