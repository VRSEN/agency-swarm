...
| `SendMessageHandoff`        | Enables unidirectional delegation of tasks or entire interaction flows to specialized agents. When a handoff occurs, the receiving agent takes over the interaction.                                                                        | Use for routing queries to specialized agents or sequential workflows where control should transfer completely. | [link](https://github.com/VRSEN/agency-swarm/blob/main/agency_swarm/tools/send_message/SendMessageHandoff.py)       |

**To use any of the pre-made `SendMessage` classes**, simply put it in the `send_message_tool_class` parameter when initializing individual `Agent` instances:
...
That's it! Now, your agents will use your own custom `SendMessageHandoff` class for communication.
...
- **Configuration**: Set `send_message_tool_class=SendMessageHandoff` when creating the agent. The library will automatically detect this and configure handoffs using the OpenAI SDK internally.
...
By default, Agency Swarm uses the following `FunctionTool`-based implementation for communication:
...
class SendMessage(FunctionTool):
    ...
    def __init__(self, sender_agent: "Agent", recipient_agent: "Agent", tool_name: str):
            ...
            "properties": {
                "my_primary_instructions": {
                    ...
                    "description": (
                        ...
                        "first break them down into smaller steps yourself."
                    ),
                },
                "message": {
                    "type": "string",
                    "description": (
                        ...
                        "what the task entails, rather than providing exact instructions."
                    ),
                },
                "additional_instructions": {
                    ...
                    "description": (
                        ...
                        "recipient agent to complete the task."
                    ),
                },
            },
    ...
    async def on_invoke_tool(self, wrapper, arguments_json_string: str) -> str:
