...
| `Handoff`                   | Enables unidirectional delegation of tasks or entire interaction flows to specialized agents. When a handoff occurs, the receiving agent takes over the interaction. (`SendMessageHandoff` is deprecated.)                                                                        | Use for routing queries to specialized agents or sequential workflows where control should transfer completely. | [link](https://github.com/VRSEN/agency-swarm/blob/main/src/agency_swarm/tools/send_message.py)       |
...
Agent-level `send_message_tool_class` is deprecated—attach custom tools to the specific flow instead.
