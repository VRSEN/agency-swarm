...
- **How it Works**: From the perspective of the underlying language model, a handoff is treated like a special tool (e.g., `transfer_to_BillingAgent`). To enable handoffs, you configure an agent with the `send_message_tool_class=SendMessageHandoff` parameter. The agent's description will be used as the handoff tool instructions. Additionally, unless disabled, it will add a special system message to indicate to the agent that a handoff has happened. This approach helps to reduce hallucinations that often appear on chat transfers.
...
You can customize the reminder message or disable it entirely by defining a custom Handoff class as follows:
