...
<Accordion title="Prerequisites" defaultOpen={true}>
  To get started with observability features, install the necessary dependencies:

  ```bash
  # Base observability features
  pip install "agency-swarm[observability]"
  
  # For AgentOps integration
  pip install "agency-swarm[agentops]"
  
  # For Langfuse integration
  pip install "agency-swarm[langfuse]"
  
  # Or install all observability tools
  pip install "agency-swarm[agentops,langfuse]"
  ```
</Accordion>
