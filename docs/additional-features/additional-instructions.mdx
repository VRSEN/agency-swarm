...
To use **additional instructions i**n widgets, simply add them in the **third parameter** in the ChatComponent ****`init()` function like below.
...
![widget-screenshot.png](/images/additional-onstrcutions-example.webp)
### CustomGPTs
...
For web api, simply use `additionalInstructions` in `get_completion` POST endpoint. For more details, see: [Web API](/references/web-api)
