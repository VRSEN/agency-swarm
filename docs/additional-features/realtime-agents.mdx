...
Agency Swarm provides a `run_realtime` helper that creates the FastAPI server, connects to OpenAI Realtime, and exposes optional Twilio bridges without manual WebSocket endpoints.
...
- When using the bundled CLI client, audio runs at 24 kHz, matching the default realtime voice output.
