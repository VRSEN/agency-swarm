...
The `raise_input_guardrail_error` parameter (alias: `throw_input_guardrail_error`) controls how guidance is delivered.

| Mode | `raise_input_guardrail_error` | Caller Sees | Persisted As | Use Case |
|------|-------------------------------|-------------|--------------|----------|
| **Non-strict** | `False` (default) | Guidance as `final_output` | Assistant message | Conversational flows, agent-to-agent |
...
For output guardrail history entries, see [Output Guardrails](/additional-features/guardrails/output-guardrails).
