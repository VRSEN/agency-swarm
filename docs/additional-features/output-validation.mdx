...
In most cases, you'll only need the `response_text` parameter, which contains the agent's latest response as a simple text string if `output_type` is not set or a BaseModel subclass, if you have set a custom `output_type`. This allows you to check the response against specific criteria.
