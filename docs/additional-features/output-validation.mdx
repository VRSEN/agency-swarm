...
Unlike output guardrails, if an input guardrail is triggered, the error will be immediately returned back to the user/caller agent.     
...
Each triggered guardrail will also leave a corresponding system message containing error message inside of a chat history.
...
<Warning>
Due to the nature of Handoffs, using SendMessageHandoff for agent-to-agent communication will bypass the input guardrails set between agents.
</Warning>
