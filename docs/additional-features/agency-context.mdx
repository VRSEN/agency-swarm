...
If you want a reminder pattern, see the [Contextual Reminders Example](https://github.com/VRSEN/agency-swarm/blob/main/examples/contextual_reminders.py). It stores reminder state in agency context, prepends a `system` item before each user turn, and shows how to trigger a stronger reminder after every 15 tool calls.
