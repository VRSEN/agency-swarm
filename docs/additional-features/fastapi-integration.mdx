...
- **ChatKit Protocol:**
  When `enable_chatkit=True`, adds `/{agency_name}/chatkit` endpoint for [OpenAI ChatKit](https://platform.openai.com/docs/guides/chatkit) UI integration. See [ChatKit Integration](#chatkit-integration) below.
...
[OpenAI ChatKit](https://platform.openai.com/docs/guides/chatkit) is a React-based chat UI that you can use with Agency Swarm.

```python
from agency_swarm import Agency, Agent, run_fastapi

def create_agency(**kwargs):  # Accept kwargs from run_fastapi
...
This exposes `/{agency_name}/chatkit` endpoint that speaks the ChatKit protocol.

### Architecture

```
┌─────────────────┐     ┌──────────────────┐     ┌─────────────────┐
│  ChatKit React  │────▶│  FastAPI Server  │────▶│  Agency Swarm   │
...
**How it works:**
1. **ChatKit frontend** sends messages with a `thread_id` to identify the conversation
2. **FastAPI endpoint** maintains conversation history in memory, keyed by `thread_id`
3. **On each request**, full history is passed to the agent as a message list
4. **Responses stream** back to ChatKit using Server-Sent Events (SSE)

<Note>
The default implementation uses **in-memory storage** (lost on restart). For production, implement custom persistence by modifying `_thread_storage` in `chatkit_handlers.py` to use your database.
</Note>

### Using with ChatKit Frontend

Point any ChatKit frontend to your backend by configuring the API URL:

```typescript
// In your ChatKit React app
const chatkit = useChatKit({
  api: { url: "http://localhost:8000/my_agency/chatkit" },
});
```

Or use a Vite proxy to avoid CORS:
