...
```json
{
  ...
  "new_messages": [
    ...
    },
    {
      "type": "message",
      "role": "system",
      "message_origin": "provider_raw_response_snapshot",
      "content": "[RAW_RESPONSE_SNAPSHOT]\\n{\"response_id\":\"resp_123\",\"output\":[...],\"usage\":{...}}"
    }
  ],
...
`new_messages` now always includes a system snapshot message per provider response (`message_origin = "provider_raw_response_snapshot"`), so reloading only `new_messages` from storage still preserves full OpenAI output metadata (web/file search calls, annotations, tool outputs, and usage payload).
...
event: messages
data: {"new_messages": [...], "run_id": "...", "cancelled": false, "usage": {...}, "raw_responses": [...], "file_ids_map": {...}}
