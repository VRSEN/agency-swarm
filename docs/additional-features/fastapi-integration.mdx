...
`new_messages` now always includes a structured snapshot item per provider response (`message_origin = "provider_raw_response_snapshot"`), so reloading only `new_messages` from storage still preserves full OpenAI output metadata (web/file search calls, annotations, tool outputs, and usage payload) without string parsing.
