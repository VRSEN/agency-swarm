...
- Defers OAuth MCP discovery in SaaS mode until the model explicitly invokes `activate_mcp_tools` (so first user messages can complete without forcing auth)
...
<Note>
For OAuth-protected FastMCP servers, schema discovery (`list_tools`) is auth-protected. That means OAuth may start before model streaming begins.
</Note>
