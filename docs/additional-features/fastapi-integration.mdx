...
- Defers OAuth MCP discovery in SaaS mode until the model explicitly invokes `authenticate_mcp_server(server_name)` (so first user messages can complete without forcing auth)
...
You do not need custom handlers. FastAPI wiring is request-scoped and carries the OAuth prompts automatically.

<Note>
For OAuth-protected FastMCP servers, schema discovery (`list_tools`) is auth-protected. In SaaS mode, this discovery step is deferred until the model invokes `authenticate_mcp_server(server_name)`, so first-message streaming can start without forcing OAuth.
</Note>
