...
For `HostedMCPTool`, FastAPI cannot infer whether a remote MCP server is public or OAuth-protected. To enable hosted OAuth, wrap only the protected tool with `enable_hosted_mcp_tool_oauth(...)` and leave `tool_config.authorization` empty. `run_fastapi(...)` will create an in-memory registry by default, or you can pass `oauth_registry=OAuthStateRegistry()` to share state across workers. Public hosted MCP servers should stay unwrapped.
