...
`run_fastapi()` exposes a single OpenAPI document that covers every agency and tool endpoint. You can capture the schema before launching the server by requesting the FastAPI app and calling `openapi()`:

```python
import json

app = run_fastapi(
    agencies={"test_agency_1": create_agency_1, "test_agency_2": create_agency_2},
    tools=[example_tool, test_tool],
    return_app=True,
    app_token_env="",
)
...
print(json.dumps(openapi_doc["paths"]["/tool/example_tool"], indent=2))
```

When you deploy the server, the same schema is available over HTTP at `/openapi.json`, and the interactive Swagger UI at `/docs` reflects the exact request bodies for every tool—whether it was created with `@function_tool` or inherits from `BaseTool`. Share those endpoints with downstream clients to keep documentation and runtime behavior in sync.
