...
If you use durable memory, pass `user_id` and `session_id` as top-level request fields. Use `memory_identity` only for advanced overrides, and keep it separate from `user_context`.

<Warning>
If your FastAPI server accepts requests from untrusted clients, bind `user_id` and `memory_identity` from authenticated server-side state. Do not let browsers or third-party callers choose durable-memory identity values directly.
</Warning>
