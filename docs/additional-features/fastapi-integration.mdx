...
- **Agency Endpoints:**
  ...
  - `/your_agency_name/cancel_response_stream` (POST, cancel active stream)
...
Optionally include `cancel_mode`, which when set to:        
    - immediately - stops the run immediately and returns all messages that were fully generated by then. Will discard the message that is currently being generated.
    - after_turn - stops after current run is fully finished processing. 
