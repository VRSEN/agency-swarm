...
- `/test_agency/cancel_response_stream`
...
<Accordion title="Turning usage tracking on/off" defaultOpen={false}>
...
**Implementation:** `src/agency_swarm/integrations/fastapi_utils/endpoint_handlers.py`
...
- `/test_agency_1/cancel_response_stream` (standard mode only; see AG-UI note below)
- `/test_agency_1/get_metadata`
- `/test_agency_2/get_response`
- `/test_agency_2/get_response_stream`
- `/test_agency_2/cancel_response_stream` (standard mode only; see AG-UI note below)
...
- **Agency Endpoints (standard mode):**
...
<Note>
Stream cancellation uses an in-memory registry per process. It requires single-worker deployments (e.g., uvicorn workers=1, the default), otherwise cancel requests routed to a different worker will return 404.
</Note>
...
Call the cancel endpoint with the `run_id` received from the streaming response. 
...
### 2. Automatic Cancellation on Disconnect

When a client disconnects (tab close, refresh, network failure), the stream is automatically cancelled.

### Streaming Response with run_id

The streaming endpoint sends a `meta` event first containing the `run_id`:
...
event: messages
data: {"new_messages": [...], "run_id": "...", "cancelled": false}

event: end
data: [DONE]
```

Store the `run_id` from the `meta` event to use with the cancel endpoint.

---

### OAuth-enabled agencies

If any agent uses `MCPServerOAuth` (or you have a `HostedMCPTool` without `tool_config.authorization`), `run_fastapi` automatically:
- Adds `/auth/callback` and `/auth/status/{state}` routes
- Emits `event: oauth_redirect` with `{state, server, auth_url}` on `/get_response_stream` when a user must authorize (and `event: oauth_status` after the callback completes)
- Reads `X-User-Id` (if provided) to store tokens per user instead of the shared `default` bucket
- Keeps OAuth state in an in-process registry; for multi-worker deployments, pass a shared registry (e.g., Redis-backed) via `run_fastapi(oauth_registry=...)` or use sticky routing.
- Requires streaming: `POST /{agency}/get_response` returns `400` when OAuth-enabled MCP servers or hosted MCP tools are present. Use `POST /{agency}/get_response_stream` (or `enable_agui=True`) and buffer the stream if you need a single final response.

You do not need custom handlers—the built-in streaming endpoint carries the OAuth prompts and finishes the run after the callback completes.

<Note>
Set `redirect_uri` on `MCPServerOAuth` to the publicly reachable URL that forwards to the FastAPI `/auth/callback` route (e.g., your ingress hostname plus `/auth/callback`). You can also set `OAUTH_CALLBACK_URL` or `<SERVER>_REDIRECT_URI` as a fallback. The default (`http://localhost:8000/auth/callback`) matches the FastAPI server's default port; override it if your FastAPI host/port differs and route that path back to the issuing pod (sticky routing or shared state) or the OAuth flow will time out.
</Note>
