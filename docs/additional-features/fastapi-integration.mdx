...
<Note>
FastAPI resolves most `$defs` generated by BaseTool and FunctionTool schemas. Schemas that rely on `oneOf`, `allOf`, or `anyOf` are currently served through the generic JSON handler; document those manually or keep the tool definition simple when you need an explicit schema.
</Note>
