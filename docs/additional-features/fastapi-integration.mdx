...
- Agencies are served at:
  ...
  - `/your_agency_name/get_logs` (GET; when `enable_logging=True`)
...
Then pass file paths in `file_urls` (supports both local and absolute paths):

```json
{"file_urls": {"doc.pdf": "/data/uploads/doc.pdf"}}
```

Invalid paths return HTTP 404 error.
