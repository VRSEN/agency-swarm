...
- Keeps OAuth state in an in-process registry. For multi-worker deployments, pass an external shared registry implementation via `run_fastapi(oauth_registry=...)`, or use sticky routing or a single worker.
...
For `HostedMCPTool`, FastAPI cannot infer whether a remote MCP server is public or OAuth-protected. To enable hosted OAuth, wrap only the protected tool with `enable_hosted_mcp_tool_oauth(...)` and leave `tool_config.authorization` empty. `run_fastapi(...)` creates an in-memory registry by default. For multiple workers, pass an external shared registry implementation, or use sticky routing or a single worker. Public hosted MCP servers should stay unwrapped.
