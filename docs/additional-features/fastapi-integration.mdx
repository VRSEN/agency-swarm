...
If you use durable memory, keep `user_id`, `session_id`, and `memory_identity` separate from `user_context`.

<Warning>
`run_fastapi()` blocks raw client-supplied durable-memory identity by default.

- Use `memory_identity_resolver` to bind trusted durable-memory identity from your server auth/session layer.
- Use `allow_client_memory_identity=True` only when the caller is fully trusted.
</Warning>
...
### Binding durable memory on the server

Use `memory_identity_resolver` when your FastAPI app authenticates users and you want durable-memory reads to follow that trusted identity.

```python
from agency_swarm import Agency, Agent, MemoryIdentity


def bind_memory_identity(http_request, _request):
...
agency.run_fastapi(memory_identity_resolver=bind_memory_identity)
```

<Note>
When `memory_identity_resolver` is present, raw client `user_id`, `session_id`, and `memory_identity` fields are ignored for durable-memory reads.
</Note>
...
| `allow_client_memory_identity` | boolean | `false` | Trust `user_id`, `session_id`, and `memory_identity` from raw HTTP bodies. Leave this off for browser or third-party clients. |
| `memory_identity_resolver` | callable | `None` | Server-side hook for binding durable-memory identity from trusted request state. |
...
<Note>
Durable memory writes also use a process-local background queue in v1. If you enable durable memory, keep each worker on its own journal path or run a single FastAPI worker.
</Note>
