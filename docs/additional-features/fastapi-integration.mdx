...
- allowed_local_file_dirs (default: `None`) - Optional allowlist of directories for local `file_urls`. When omitted, local paths are rejected.
...
# File attachments as URLs or absolute local paths. Requires the server to be configured with allowed_local_file_dirs for local paths.
# Provide as {"filename_1":"url_or_path", "filename_2":"url_or_path", ...}
...
Responses include a `usage` object with token counts and cost when `ModelSettings(include_usage=True)` is set (default). For streaming, the final `event: messages` payload includes `usage`.
...
- Remote files: downloaded to a temp file named after your provided key, uploaded to OpenAI, and waited on until processed.
- Local absolute paths (when `allowed_local_file_dirs` is set on `run_fastapi`): uploaded directly after checking the path is inside the allowlist. Paths outside the allowlist or missing the allowlist return an error.
