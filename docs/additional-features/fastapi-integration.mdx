...
| `enable_chatkit` | boolean | `false` | Add `/{agency_name}/chatkit` for ChatKit clients. |
...
- Agencies are served at:
  ...
  - `/your_agency_name/chatkit` (POST; when `enable_chatkit=True`)
...
- ChatKit-specific setup lives on [ChatKit Integration](/additional-features/chatkit-integration).
