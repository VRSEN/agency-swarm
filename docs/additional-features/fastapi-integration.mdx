...
user_context: dict[str, Any] = None  # Optional per-request Agency Context merged with the agency's user_context
```

`user_context` lets you pass structured Agency Context data from your frontend (including AG-UI payloads) without exposing it to the LLM. It is merged with any `user_context` set on the agency instance for that run so tools and hooks can read it.
