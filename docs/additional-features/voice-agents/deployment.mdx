...
Use this guide once your agent is ready and you want to host a realtime bridge or connect phone infrastructure. It builds on the [Overview](./overview) and assumes you installed `agency-swarm[fastapi]`. Add `agency-swarm[voice]` when you need local audio capture or playback utilities (they remain separate so projects can choose only what they need).
