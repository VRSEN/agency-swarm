...
Example persistence callbacks in a FastAPI application:

<Tabs>
<Tab title="v1.x (Current)" defaultOpen={true}>
...
async def handle_chat(chat_id: str, message: str):
    ...
    response = await agency.get_response(message, recipient_agent="CEO")
    return {"response": response.final_output}
```

</Tab>
<Tab title="v0.x (Legacy)">

```python
@app.post("/chat/{chat_id}")
def handle_chat(chat_id: str, message: str):

    # define callbacks the same way
    # ...
...
<Warning>
**v1.x Change:** The old `threads_callbacks` parameter is deprecated. Use `load_threads_callback` and `save_threads_callback` instead for better clarity and functionality.

**Data Structure Change:** v1.x saves complete conversation history for each thread, while v0.x only saved thread IDs. When migrating to v1.x, test in a staging environment first as this may impact database storage requirements and user privacy considerations.
</Warning>
...
<Warning>
**Note:** Agent settings persistence via `settings_callbacks` is deprecated in v1.x. Agent configurations should be managed through code and version control rather than dynamic persistence. This approach provides better reliability and deployment consistency.
</Warning>

<Tabs>
<Tab title="v0.x (Legacy)" defaultOpen={true}>
...
<Tab title="v1.x (Current)">

Agent settings persistence is deprecated in v1.x. Agent configurations should be managed through code and version control rather than dynamic persistence.

```python
# v1.x approach: Define agents with fixed configurations
from agency_swarm import Agent, Agency

ceo = Agent(
    name="CEO",
    description="Chief Executive Officer",
    instructions="Your fixed instructions here...",
    model="gpt-4o"
)
