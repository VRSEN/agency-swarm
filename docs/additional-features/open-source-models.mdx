...
Here both claude and gemini agents will be able to use their native search tools, which are similar to openai's WebSearch() tool. Consider checking out [LiteLLM's documentation](https://docs.litellm.ai/docs) to find a full list of supported tools.
...
- **Potential library conflicts**: Agents SDK is still a fairly new framework which is being actively developed and improved. Due to that, there might be potential conflicts between litellm and openai-agents libraries on recent releases.
