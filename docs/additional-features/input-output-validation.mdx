...
description: "Configure validation for tool inputs, agent prompts, and final responses."
icon: "shield-check"
---
...
Agency Swarm supports two complementary validation mechanisms: Pydantic tool validators enforce tool argument schemas, and Agents SDK guardrails review agent inputs and outputs before they are delivered.

- Tool validators ([Pydantic `field_validator`](https://docs.pydantic.dev/latest/usage/validators/#field-validators) / [`model_validator`](https://docs.pydantic.dev/latest/usage/validators/#model-validators)):
  - `field_validator`: Validate individual fields independently.
  - `model_validator`: Perform complex checks involving multiple fields.
- Agent guardrails ([Agents SDK guardrails overview](https://openai.github.io/openai-agents-python/guardrails/)):
  - `input_guardrails`: Screen user messages before they reach the agent.
  - `output_guardrails`: Review agent responses before they reach users or other agents.

### Agent Response Validator

In Agency Swarm, validation is handled through **guardrails** using the Agents SDK, which provides more flexible validation for both input and output.
...
Agent's configuration includes an optional parameter called `throw_input_guardrail_error`, which defines how guardrail guidance is surfaced and persisted:
```python
agent = Agent(
    name="CustomerSupportAgent",
    instructions="You are a helpful customer support agent.",
...
```
| `throw_input_guardrail_error` | Caller sees                                                                                        | Persisted entry                                                |
|-------------------------------|----------------------------------------------------------------------------------------------------|----------------------------------------------------------------|
| `False` (default)             | Guardrail text returned as `final_output` (non-streaming) or as the `message_output_created` event | Assistant message (`message_origin="input_guardrail_message"`) |
| `True`                        | `InputGuardrailTripwireTriggered` exception                                                        | System message (`message_origin="input_guardrail_error"`)      |

For example, using the agent above with the default setting:
```python
response = await agency.get_response("Hello!")
...
When streaming is enabled and `throw_input_guardrail_error=False` (the default), the final event looks like this:
```text
RunItemStreamEvent(
...
**When to use:** Keep the default (`False`) to keep the experience fluid—callers see the guidance inline as if it came from the agent, and you don't need extra exception handling. Switch to `True` when you prefer to abort the turn, raise `InputGuardrailTripwireTriggered`, and persist only the system guidance entry for stricter flows.
...
Each triggered guardrail leaves exactly one guidance entry inside the chat history. With `throw_input_guardrail_error=False`, that entry is the impersonated assistant message (`input_guardrail_message`) and its text matches what the caller receives. With `throw_input_guardrail_error=True`, the guardrail raises an exception and only the system guidance entry (`input_guardrail_error`) remains.
...
Each guardrail trigger is recorded in the chat history with a guidance entry. Every entry carries a `message_origin` field so you can determine which guardrail fired (`output_guardrail_error`, `input_guardrail_error`, or `input_guardrail_message`).

| `throw_input_guardrail_error` | Streaming Event                             | Persisted Entry                                               |
|-------------------------------|---------------------------------------------|---------------------------------------------------------------|
| `False` (default)             | `message_output_created` with guidance text | Assistant message, `message_origin="input_guardrail_message"` |
| `True`                        | `{"type": "error", "content": guidance}`    | System message, `message_origin="input_guardrail_error"`      |
