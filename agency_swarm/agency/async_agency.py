...
from agency_swarm.threads.async_thread import AsyncThread
...
class AsyncAgency:
    ...
    def demo_gradio(self, height=600):
        ...
        with gr.Blocks() as demo:
            ...
            async def bot(history):
                ...
                gen = await self.get_completion(message=history[-1][0])
    ...
    async def run_demo(self):
        while True:
            ...
            try:
                gen = iter(await self.main_thread.get_completion(message=text))
                while True:
                    message = next(gen)
                    message.cprint()
    ...
    def _add_agent(self, agent: AsyncAgent):
        if not agent.id:
            # assign temp id
    ...
    def _create_send_message_tool(self, agent: AsyncAgent,
                                  recipient_agents: List[AsyncAgent]) -> Type[ASYNCBASETOOL_SUBCLASS]:
        ...
        class SendMessage(AsyncBaseTool):
