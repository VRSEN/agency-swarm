...
class SendKeys(BaseTool):
    ...
    elements_and_text: dict = Field(
        title="Elements and Text",
        description="A dictionary where the key is the sequence number of the element and the value is the text to be typed into that element.",
        examples=[{"52": "johndoe@gmail.com", "53": "password123"}]
    )

    def run(self):
        wd = get_web_driver()
        if 'input' not in self.shared_state.get("elements_highlighted", ""):
            raise ValueError("Please highlight input elements on the page first by outputting '[highlight input elements]' message. Current elements highlighted are: " + self.shared_state.get("elements_highlighted", ""))

        all_elements = wd.find_elements(By.CSS_SELECTOR, '.highlighted-element')

        i = 0
        try:
            for key, value in self.elements_and_text.items():
                key = int(key)
                ...
                if i == len(self.elements_and_text) - 1:
